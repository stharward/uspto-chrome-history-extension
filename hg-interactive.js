function generateHistoryPage() {
  ...
  h.textContent = "Prior Art Web Search History";
  ...
  hc = hr.insertCell(2);
  hc.textContent = 'query';
  for (var i = 0; i < entries.length; ++i) {
    r = t.insertRow(i+1);
    tc = r.insertCell(0);
    tc.textContent = entries[i]['timestamp'];
    tc = r.insertCell(1);
    tc.textContent = "database";
    tc = r.insertCell(2);
    tc.textContent = entries[i]['title'];
    //tc.textContent = historyData[i]['search string'];
  }
